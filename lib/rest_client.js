...
module.exports.RestClient = function (options, req) {
  let instance = {};
  ...
  function apiCall(request_data, request_token = '') {
    ...
    if (req && req.headers) {
      clientAppVersion = req.headers['app-version'];
      clientAppInstance = req.headers['instance-id'];
    }

    if (clientAppInstance) {
      headers['instance-id'] = clientAppInstance;
    }

    if (clientAppVersion) {
      headers['app-version'] = clientAppVersion;
    }
    ...
    return new Promise(function (resolve, reject) {
      request({
        ...
        if (error) {
          ...
          reject({ errorMessage, code: body ? body.code : response.statusCode });
        }
        resolve(body);
      });
    });
  ...
  function createUrl(resourceUrl) {
    let [url, query] = resourceUrl.split('?');
    const lastUrlSymbol = url[url.length - 1];

    if (lastUrlSymbol !== '/'){
      url += '/';
    }

    let targetUrl = servelrUrl + '/' + url;
