...
module.exports.RestClient = function (options) {
  ...
  function apiCall(request_data, user_agent, request_token = '') {
    const authorizationToken = request_token
         ? { 'Authorization': 'Bearer ' + request_token }
          : oauth.toHeader(oauth.authorize(request_data, token));
    const userAgentToken = user_agent ? { 'user-agent': user_agent } : undefined;

    let headers = { ...authorizationToken };

      if (userAgentToken) {
        headers = { ...headers, ...userAgentToken };
      }
    ...
    return new Promise(function (resolve, reject) {
      request({
        ...
        if (error) {
          ...
          reject({ errorMessage, code: body.code || response.code });
        }
        resolve(body);
      });
    });
  ...
  instance.consumerToken = function (login_data, user_agent) {
    return apiCall(
      {
  ...
  instance.get = function (resourceUrl, user_agent, request_token = '') {
    const request_data = {
      url: createUrl(resourceUrl),
      method: 'GET'
    };
    return apiCall(request_data, user_agent, request_token);
  }
  ...
  instance.post = function (resourceUrl, data, user_agent, request_token = '') {
    const request_data = {
      url: createUrl(resourceUrl),
      method: 'POST',
      body: data
  ...
  instance.put = function (resourceUrl, data, user_agent, request_token = '') {
    const request_data = {
      url: createUrl(resourceUrl),
      method: 'PUT',
      body: data
  ...
  instance.delete = function (resourceUrl, user_agent, request_token = '') {
    const request_data = {
      url: createUrl(resourceUrl),
      method: 'DELETE'
    };
